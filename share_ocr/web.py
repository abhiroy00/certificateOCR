...
async function renderPreviews(){const token=++previewToken,strip=document.querySelector('#previews');for(const u of previewUrls)URL.revokeObjectURL(u);previewUrls=[];strip.replaceChildren();const list=[...files.files,...folderFiles.files].filter(f=>/\.(pdf|png|jpe?g|tiff?|webp|bmp)$/i.test(f.name)),MAX=200;strip.hidden=!list.length;for(let i=0;i<Math.min(list.length,MAX);i++){const f=list[i],url=URL.createObjectURL(f);previewUrls.push(url);const a=document.createElement('a');a.className='thumb';a.href=url;a.target='_blank';a.rel='noopener';a.title=f.name;let media;if(/\.pdf$/i.test(f.name))media=document.createElement('canvas');else if(/\.tiff?$/i.test(f.name))media=placeholder('TIFF');else{media=document.createElement('img');media.src=url;media.alt=f.name;media.loading='lazy'}const cap=document.createElement('span');cap.textContent='#'+(i+1);a.append(media,cap);strip.append(a)}if(list.length>MAX){const more=document.createElement('p');more.className='muted';more.textContent=`+${list.length-MAX} more file(s)`;strip.append(more)}for(const a of strip.querySelectorAll('a.thumb')){const c=a.querySelector('canvas');if(!c)continue;if(token!==previewToken)return;try{await pdfThumb(a.href,c)}catch(_){if(token===previewToken)c.replaceWith(placeholder('PDF'))}}}
