...
let rowsRevision=null;
async function refreshRows(){try{const r=await fetch('/api/rows');if(!r.ok)return;const d=await r.json();if(d.revision===rowsRevision)return;rowsRevision=d.revision;const body=document.querySelector('#result-rows');body.replaceChildren();document.querySelector('#record-count').textContent=d.total+' records';for(const x of d.rows){const tr=document.createElement('tr');for(const key of ['idx','file','company','folio','regfolio','cert','holder','shares','facevalue','sharetype','distinctive','date','latest','latestfolio','foliohistory','holderhistory','remarks','flags']){const td=document.createElement('td'),content=document.createElement('div');content.className='cell-text';if(key==='file'){const a=document.createElement('a');a.href=x.file_url;a.target='_blank';a.rel='noopener';a.textContent=x.file||'';content.append(a)}else content.textContent=x[key]??'';content.title=content.textContent;td.append(content);tr.append(td)}body.append(tr)}}catch(_){}}
